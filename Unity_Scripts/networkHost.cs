...
public static class Extensions
{
    ...
    public static string FilterString(this string str, List<string> stringsToRemove)
    {
        stringsToRemove.ForEach(c => str = str.Replace(c.ToString(), String.Empty));
        return str;
    }
}
        ...
        try
        {
            if (newSock.Available > 0)
            {
               ...
               //quatern = new Quaternion(0, 0,float.Parse(EulerAngles[2]) , float.Parse(EulerAngles[3]));
                quatern = new Quaternion(float.Parse(EulerAngles[0]), float.Parse(EulerAngles[2]), -1*float.Parse(EulerAngles[1]), float.Parse(EulerAngles[3]));
